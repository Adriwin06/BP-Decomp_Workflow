...
- **Reconstruction Quality & Type Recovery (CRITICAL):** The goal is to reconstruct what the original C++ source code **looked like**, not to translate raw decompiler outputs literally.
  ...
  - **USE DECFIGS DWARFDUMP HINTS:** For DecFIGS-backed TUs, consult `references/DecFIGS/dwarfdump/` when present (auto-surfaced by `work show --full`) for C++-shaped DWARF declarations: class/struct outlines, enum values, member names/types, globals, function signatures, and local-variable names/types. Treat this as reconstruction guidance, not complete source code. It is not offset authority; verify member placement and behavior against X360 pseudocode/asm, and prefer Feb-2007 leaked source where it overlaps.
